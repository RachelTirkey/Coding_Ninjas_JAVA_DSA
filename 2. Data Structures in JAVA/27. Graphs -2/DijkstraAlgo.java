...
public class DijkstraAlgo {
	private static void dijkstra(int[][] adjacencyMatrix) {
		...
		for (int i = 0; i < v - 1; i++) {
			...
			// Explore neighbors
			for (int j = 0; j < v; j++) {
				// here we take those nodes which are adjacent/neighbours and whcih are mot
				// visisted
		...
		// Print the shortest distance of all nodes from souce that is 0 in this case.
		for (int i = 0; i < v; i++) {
	...
	private static int findMinVertex(int[] distance, boolean visited[]) {
		...
		for (int i = 0; i < distance.length; i++) {
			// in case of || if minVertex == -1 is true it wont go the 2nd case in case of
			// Or
